...
# استبدال وظيفة فحص Gmail بوظيفة فحص Hotmail من script2
    ...
    try:
      ...
      if '"status":"ok"' in response:
        if "The password you entered is incorrect." in response or "Login Error: An unexpected error occurred. Please try logging in again." in response:
            gg += 1
            PLAY()
            check_hotmail(email)  # تغيير من check_gmail إلى check_hotmail
...
# مولد الأسماء القوي من Script 1 مع تعديل النطاق إلى Hotmail
def eizon_python():
    global email
    while True:
        data = {
        ...
        try:
            ...
            if username:
                ...
                email = username + eizon_domain  # استخدام hotmail.com بدلاً من gmail.com
